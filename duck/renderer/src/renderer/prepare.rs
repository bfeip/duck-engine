...
use super::{RenderContext, SceneResources};
...
impl SceneResources {
    ...
    pub fn prepare(&mut self, ctx: &mut RenderContext, scene: &mut SceneData) -> Result<()> {
        ...
        for texture in scene.textures() {
            self.gpu_textures.try_ensure(texture.id(), texture.generation(), || {
                create_texture_gpu_resources(texture, device, queue)
            })?;
        }

        // 2. Materials (face/line/point), reading uploaded textures.
        self.materials.prepare(device, &mut ctx.pipelines, scene, &self.gpu_textures)?;
        ...
        for mesh in scene.meshes() {
            self.gpu_meshes.ensure(mesh.id, mesh.generation(), || {
                MeshGpuResources::new(mesh, device)
            });
        }
        ...
        {
            self.ibl_resources.process_environment(device, queue, env_map)?;
        }
